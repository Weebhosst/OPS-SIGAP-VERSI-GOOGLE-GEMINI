...
export const AdminCheckpoints: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-[#020817] pb-10 text-slate-100">
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4 backdrop-blur-md"><div className="ops-dialog w-full max-w-3xl space-y-4 overflow-y-auto rounded-3xl border border-slate-700/90 bg-[#0f172a] p-5 shadow-2xl shadow-black/50" role="dialog" aria-modal="true" aria-labelledby="checkpoint-create-title">
          <div className="flex items-center justify-between"><div><h3 id="checkpoint-create-title" className="text-sm font-black">Tambah Checkpoint Site</h3><p className="text-[11px] text-slate-400">Checkpoint dibuat tanpa token. Generate token lalu QR setelah data lokasi benar.</p></div><button type="button" onClick={() => setShowAddModal(false)} aria-label="Tutup tambah checkpoint" className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white">✕</button></div>
      ...
      {showEditModal && selectedCp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="ops-dialog w-full max-w-md space-y-4 overflow-y-auto rounded-3xl border border-slate-700/90 bg-[#0f172a] p-5 shadow-2xl shadow-black/50" role="dialog" aria-modal="true" aria-labelledby="checkpoint-edit-title">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <h3 className="text-sm font-black text-white">Edit Checkpoint {selectedCp.code}</h3>
              <button type="button" onClick={() => setShowEditModal(false)} className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-800 hover:text-white" aria-label="Tutup edit checkpoint">
                ✕
              </button>
            </div>
      ...
      {showPrintModal && selectedCp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-4">
          <div className="ops-dialog w-full max-w-md space-y-4 overflow-y-auto rounded-2xl bg-white p-6 text-center text-slate-900 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="checkpoint-qr-title">
            <div className="flex justify-between items-center pb-2 border-b border-slate-200">
              <span id="checkpoint-qr-title" className="text-xs font-bold text-blue-600 uppercase tracking-wider">
            ...
            <div
              id="printable-card"
              className="space-y-3 rounded-2xl border-4 border-slate-900 bg-white p-5"
            >
